...
def ssh_conect(query={}):
    ...
    mongosh_cmd = f'mongosh "mongodb://administrator:password@mongo1:27017" --eval "use test2" --eval '+ query +'\n'
...
class BibliotecarioInterface(tk.Tk):
    ...
    def confirm_create(self, db_name, new_title, new_author, new_rating, new_precio, new_ano, new_genero, new_disponible, new_estado, create_dialog):
        ...
        book_data = {"Id_book": 21,"title": new_title,"author": new_author,"Reviews": 0,"Price": new_precio,"Year": new_ano,"Genre": new_genero,"Available": new_disponible,"Status": new_estado,"CDU": 0}
        ssh_conect(query=r'"db.'+db_name+'.insert('+str(book_data)+')"')
        # self.database["Libro"].append(book_data)
        # with open(dabasepath, "w") as file:
        #     json.dump(self.database, file, indent=4)
    ...
    def db_checker(self, item_id, db_name):
        try:
            item_id = int(item_id)
        except ValueError:
            messagebox.showerror("Error", "Ingrese un ID válido.")
        ...
        # Buscar el elemento en la base de datos
        if db_name in self.database:
            return self.database[db_name][item_id-1]
        else:
            return []
    ...
    def show_info(self, item_id, db_name, input_dialog):
        # mongosh "mongodb://administrator:password@mongo1:27017/" --eval "use test2" --eval "db.Libros.find({Id_book:2})"
    ...
    def confirm_update(self, db_name, id, item_id, new_title, new_author, new_rating, new_state, edit_dialog):
        # db.Libros.update({_id: ObjectId("64fc8b811e8aa3a54c696029")}, {$set: {title: "Libro de la Jungla"}})
        # db.Libros.replaceOne({"Id_book" : 1 }, {Id_book: idbook, title: 'NombreLibro', author: 'Autor 50', Rating: 2, Reviews: 19, Price: 1800, Year: 1877, Genre: 'Fiction', Available: false, Status: 'Extraviado', CDU: 30}
...
class ConsultasInterface(tk.Tk):
    def __init__(self):
        ...
        self.usuarios_button = tk.Button(self, text="Consultar Usuarios", command=lambda: self.crud('Usuario'))
